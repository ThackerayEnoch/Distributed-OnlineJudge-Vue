<template>
    <AccessDenied v-if="isAccessDenied" :message="message" btnLabel="返回作业" btnTo="/homeworks" btnIcon="pi pi-book" />
    <div v-else class="felx flex-col w-full dark:bg-gray-800">
        ...
        <div class="flex" style="align-items: flex-start;">
            ...
            <div class="w-full p-6 ml-0 my-3 flex bg-white shadow-lg flex flex-col dark:bg-gray-800 overflow-x-auto">
                <router-view name="homeworkTab" :description="homework.description" :problems="problems" />
            </div><!--显示不同导航栏内容-->
        </div>
    </div>
...
async function localHomeworkDetail(homeworkId: number) {
    getHomeworkSummary(homeworkId).then(res => {
        ...
        if (error.code === ProblemStatus.ACCESS_DENIED) {
            ...
            globalMessage.warn('提示', '您没有权限查看此作业');
            return;
